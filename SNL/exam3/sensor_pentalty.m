function [x1,x,XX,sigma_opt,round]= sensor_pentalty(X0,sigma0,terminal,a,distance3)  %X0:(n,N), B:(n,N)
...
%              end
         ...
         for i =1:N
                 for m=1:n
                       ...
                       if -x(m,1,j)+x_min>0
                          ...
                          ran_a = -1;
                          ran_b = 1;
                          sgn1(m,i) = ran_a + (ran_b-ran_a) * rand(1,1);
                          end
                       end
                           if  x(m,1,j)-x_max<0
                          sgn2(m,i)=0;
                       ...
                       if x(m,1,j)-x_max>0
                          ...
                          ran_a = -1;
                          ran_b = 1;
                          sgn2(m,i) = ran_a + (ran_b-ran_a) * rand(1,1);
                          end
                       end
                   end
         end              
         ...
         for i =1:N
              BR(:,i)=x(:,i,j)-step*(dx(:,i)-10*sgn1(:,i)-10*sgn2(:,i));
         end
